...
def process_data(url, user, password, host, port, db, table_name, chunk_size=100000):
    ...
    if url.endswith(".parquet"):
        ...
        raise ValueError(
            "Unsupported file format. Only '.parquet' and '.csv.gz' are supported."
        )
...
def ingest_parquet_to_postgresql(parquet_file, table_name, engine, chunk_size=100000):
    ...
    print(
        f"Starting ingestion of {total_rows} rows from '{parquet_file}' into table '{table_name}'."
    )
...
def ingest_csv_to_postgres(csv_file, table_name, engine, chunksize=100000):
    ...
    while True:
        try:
            ...
            t_end = time()
            print(
                f"Inserted another chunk into '{table_name}', took {t_end - t_start:.3f} seconds."
            )

        except StopIteration:
            print(
                f"Finished ingesting data into the PostgreSQL database for table '{table_name}'."
            )
            break
